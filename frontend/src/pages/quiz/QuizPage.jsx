...
export default function QuizPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const exRes = await cacheExercices(API, leconId || null);
        setExercices(exRes.data);

        if (leconId) {
          const lecRes = await cacheLeconDetail(API, leconId);
          setLeconTitre(lecRes.data.titre);
        }
  ...
  const progress = exercices.length > 0 ? ((currentIndex + (submitted ? 1 : 0)) / exercices.length) * 100 : 0;
  ...
  const handleSubmit = async () => {
    ...
    // Enregistrer la performance dans le backend (ou hors-ligne)
    const perfData = {
      eleve: user.id,
      exercice: current.id,
    ...
    try {
      if (isOnline()) {
        ...
        addPendingAction({ method: 'POST', url: '/performances/', data: perfData });
      }
    } catch {
      // Erreur réseau → sauvegarder pour synchro ultérieure
      addPendingAction({ method: 'POST', url: '/performances/', data: perfData });
    }
  };
